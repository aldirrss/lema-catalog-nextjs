...
import type { TechnicalExpert } from '@/types';
...
export default function AboutPage() {
  const { t } = useLang();
  const [experts, setExperts] = useState<TechnicalExpert[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchExperts() {
      try {
        const res = await fetch('/api/technical-experts', { cache: 'no-store' });
        ...
        console.error('Failed to load technical experts:', error);
      }
    }
  ...
  const teamMembers: TeamMember[] =
    experts.length > 0
      ? experts.map((expert) => ({
          ...
          portfolio: expert.portfolio || undefined,
          contact: expert.contact ? `mailto:${expert.contact}` : undefined,
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <p className="mt-4 text-lg" style={{ color: 'var(--text-secondary)' }}>
              Built by technical experts focused on scalable ERP solutions and long-term product quality.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            {teamMembers.map((member, index) => (
              <RevealOnScroll key={`${member.name}-${index}`} delay={index * 0.08}>
                <div className="card overflow-hidden group hover:border-brand-500 transition-all duration-300 mx-auto w-full max-w-xl">
                <div className="p-8">
                  ...
                  <div className="mt-6 text-center">
                    ...
                    <div className="mt-6 grid grid-cols-1 gap-3">
                      ...
                      {member.contact && (
                        <Link
                          href={member.contact}
                          className="flex items-center justify-center gap-2 px-4 py-2 border rounded-xl font-bold text-sm transition-colors"
                          style={{
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {SERVICES.map((s, index) => (
              <RevealOnScroll key={s.title} delay={index * 0.08}>
                <div className="card p-8 text-center hover:bg-surface-2 transition-colors">
